...
module.exports = {
  ...
  classification: {
    ...
    getCatsBooks: function (gender, type, major, minor, start,limit) {
      if (minor) {
        return `${API_HOST}/book/by-categories?gender=${gender}&type=${type}&major=${major}&minor=${minor}&start=${start}&limit=${limit}`
      } else {
        return `${API_HOST}/book/by-categories?gender=${gender}&type=${type}&major=${major}&start=${start}&limit=${limit}`
      }
    }
  },
  book: {
